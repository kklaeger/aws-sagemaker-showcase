"""
This module is based on code exported from SageMaker Data Wrangler.

The exported code was simplified and adapted for this project.
"""
import base64
import collections
import copy
...
op_1_output = s3_source(spark=spark, mode=mode, **{'dataset_definition': {'datasetSourceType': 'S3', 'name': 'german_credit.csv', 'description': None, 's3ExecutionContext': {'s3Uri': 's3://aws-sagemaker-showcase/raw/german_credit.csv', 's3ContentType': 'csv', 's3HasHeader': True, 's3FieldDelimiter': ',', 's3DirIncludesNested': False, 's3AddsFilenameColumn': False, 's3RoleArn': None, 's3CsvEncodingType': 'UTF_8', 's3SkipLines': 0, 's3MultiLine': False, 's3DataType': 'S3Prefix', 's3ManifestPlain': {'s3Uris': None}}, 'canvasDatasetMetadata': None}})
op_2_output = infer_and_cast_type(op_1_output['default'], spark=spark, **{})
op_3_output = encode_categorical(op_2_output['default'], spark=spark, **{'operator': 'One-hot encode', 'one_hot_encode_parameters': {'input_column': ['checking_status', 'credit_history', 'purpose', 'employment', 'foreign_worker', 'housing', 'job', 'other_parties', 'other_payment_plans', 'own_telephone', 'personal_status', 'property_magnitude', 'savings_status'], 'invalid_handling_strategy': 'Keep', 'drop_last': False, 'output_style': 'Columns'}, 'ordinal_encode_parameters': {'invalid_handling_strategy': 'Replace with NaN'}})

#  Glossary: variable name to node_id
#
#  op_1_output: a86cc475-e8c7-45ab-a0ca-5decbbf65ca7
#  op_2_output: 4a0d0c70-9f37-4dce-ae7f-ff9704ab5410
#  op_3_output: d4346792-33e3-4092-bf5a-152a3a8bd54a
